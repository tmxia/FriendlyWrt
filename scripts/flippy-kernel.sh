#!/bin/bash
# flippy-kernel.sh - 内核替换 + 模块注入 + boot.img 重建 + resource.img DTB 替换 + 静态验证
set -euo pipefail

VERSION="2026-09-16-v51-flippy-kernel-rename"
log()  { echo -e "\033[0;32m[flippy]\033[0m $*"; }
warn() { echo -e "\033[0;33m[flippy]\033[0m $*"; }
...
WORK_DIR=$(mktemp -d /tmp/flippy-kernel.XXXXXX)
VERIFY_FAIL=0
...
python3 - "$IMAGE_FILE" "$TARGET_DIR/kernel.img" "$SYSTEM_MAP" <<'PYEOF'
import sys, struct, os

src, dst = sys.argv[1], sys.argv[2]
sysmap_path = sys.argv[3] if len(sys.argv) > 3 and sys.argv[3] else None
...
for i in range(min(3, entry_count)):
    ...
    print(f"  [{i:2d}] magic={bytes(magic)} name='{name}' sector=0x{sector:x} size={size}")
...
assert raw[off:off+4] == b'ENTR', f"ENTR magic 丢失: {bytes(raw[off:off+4])!r}"
assert raw[off+NAME_OFFSET:off+NAME_OFFSET+len(new_name)] == new_name, "name 写入失败"
...
# 8.3 resource 分区内 rk-kernel.dtb 校验
log ""
...
for i in range(entry_count):
    ...
    if name == 'rk-kernel.dtb':
        ...
        data_off = sector * 512
        print(f'    entry[{i}] magic={bytes(magic)} name=\"{name}\" sector={sector} size={size}')
